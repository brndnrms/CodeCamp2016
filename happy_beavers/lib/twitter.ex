defmodule TwitStat.Twitter do
  ...
  def init do
    {:ok, _} = Agent.start_link fn -> @keys end, name: :keys_agent
  end

  defp setup_keys do
    [h|_] = Agent.get_and_update(:keys_agent, fn(l=[h|t]) -> {l, t ++ [h]} end)
    ExTwitter.configure h
  end

  def friend_ids(user, opts \\ []) do
    setup_keys
    ExTwitter.friend_ids(user, opts ++ [count: 5000])
  end

  def follower_ids(user, opts \\ []) do
    setup_keys
    ExTwitter.follower_ids(user, opts ++ [count: 5000])
  end

  def user_lookup(opts \\ []) do
    setup_keys
    ExTwitter.user_lookup(opts ++ [include_entities: false])
  end
